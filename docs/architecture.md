...
- Speaks Model Context Protocol to a host over **stdio** (npm package) or **Streamable HTTP** (official hosted URL `https://mcp.aureonlabs.network/mcp`).
...
Host (Cursor / Claude / other MCP host)
  → MCP JSON-RPC over stdio  OR  Streamable HTTP https://mcp.aureonlabs.network/mcp
    → mcp handlers + Zod
      → @buildaureon/sdk HTTP client
        → https://api.aureonlabs.network (mainnet default, or testnet with AUREON_NETWORK=testnet)
...
flowchart TB
  ...
  HostedURL["https://mcp.aureonlabs.network/mcp"]

  subgraph mcpPkg ["adapter"]
  ...
  API[official_api_aureonlabs_network]
  Vault[Smart_Vault]
...
The npm binary is a **local stdio adapter**. Hosts spawn it as a child process. The official hosted process is a separate Streamable HTTP server at `https://mcp.aureonlabs.network/mcp` (same tools, same SDK, same “no private key / no broadcast” rule). Do not publish **your own** stdio child as an open internet socket. Use the official hosted URL instead.
...
3. `loadConfig()` uses `resolveAureonNetworkFromEnv()` (default official API / mainnet; `AUREON_NETWORK=testnet` to stay on testnet), plus `AUREON_API_KEY` and optional `AUREON_AUTH_TOKEN`.
...
- HTTP transport to the official API (default mainnet, or testnet when `AUREON_NETWORK=testnet`).
...
| Public API `api.aureonlabs.network` | Server-side behavior on **mainnet** by omit, **testnet** when `X-Aureon-Network: testnet`; may evolve independently |
...
5. **Official hosted HTTP, or local stdio** — use `https://mcp.aureonlabs.network/mcp` for the official Streamable HTTP host. Do not expose your own stdio child on the public internet.
...
**Is MCP a second API?**  
No. It is an adapter over the same SDK client used by scripts. Hosted HTTP and local stdio are transports, not a second control plane.
...
**stdio:** the MCP child restarts; in-memory Bearer is lost. Issued key from env is reloaded.  
**hosted:** Cursor reconnects to the same URL. Open tools still work. Your wallet tools need `X-Aureon-Api-Key` again on that session.

**How many tools are there?**  
See `tools/catalog.ts` (`TOOL_COUNT`) and `tools.md`.

**Can I expose my local stdio MCP over the public internet?**  
Do not. Use the official hosted URL `https://mcp.aureonlabs.network/mcp` instead of publishing your own process.
...
`@buildaureon/mcp` is a **thin adapter** (stdio locally, Streamable HTTP on the official host): Host → MCP handlers/Zod → `@buildaureon/sdk` → official API (`https://api.aureonlabs.network`) → vault/chain. Responsibilities are split so agents get a stable tool surface while all financial intelligence and custody boundaries remain outside the MCP process.
